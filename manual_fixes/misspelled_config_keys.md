...
- `VERSION` is the specific version used for dbt v2
- e.g. `v2.0.0-previous.220`, but `https://public.cdn.getdbt.com/fs/latest.json` gives you the latest version
...
3. If it's unlikely to be a misspelling, unsupported config keys need to be moved under a `meta:` block or dbt v2 will fail to parse the user's project. If the unsupported config key is in `dbt_projects.yml` it needs to be moved under a `+meta:` block.
