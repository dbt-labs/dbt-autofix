...
def remove_unmatched_endings(content: SQLContent, config: SQLRefactorConfig) -> SQLRuleRefactorResult:
    return _RemoveUnmatchedEndingsImpl(content, config).execute()


class _RemoveUnmatchedEndingsImpl:
    ...
    def execute(self) -> SQLRuleRefactorResult:
        result_content = self._process()
        return SQLRuleRefactorResult(
            rule_name="remove_unmatched_endings",
            refactored=result_content != self.sql_str,
            refactored_content=result_content,
            original_content=self.sql_str,
    ...
    def _process(self) -> str:
        ...
        macro_stack: List[Tuple[int, int, str]] = []
...
def refactor_custom_configs_to_meta_sql(content: SQLContent, config: SQLRefactorConfig) -> SQLRuleRefactorResult:
    ...
    return _RefactorCustomConfigsToMetaSqlImpl(content, config).execute()
                ...
                if "meta" not in refactored_sql_configs:
                    ...
                    if isinstance(existing_meta, str):
                        ...
                        try:
                            ...
                            meta_dict = {
                                k: repr(v) if not isinstance(v, str) else f"'{v}'" for k, v in parsed_meta.items()
                            }
...
def move_custom_config_access_to_meta_sql(content: SQLContent, config: SQLRefactorConfig) -> SQLRuleRefactorResult:
    ...
    return _MoveCustomConfigAccessToMetaSqlImpl(content, config).execute()


class _MoveCustomConfigAccessToMetaSqlImpl:
    def __init__(self, content: SQLContent, config: SQLRefactorConfig) -> None:
        self.sql_str = content.current_str
        self.config = config
        self.schema_specs = config.schema_specs
...
def rename_sql_file_names_with_spaces(content: SQLContent, config: SQLRefactorConfig) -> SQLRuleRefactorResult:
    return _RenameSqlFileNamesWithSpacesImpl(content, config).execute()


class _RenameSqlFileNamesWithSpacesImpl:
    def __init__(self, content: SQLContent, config: SQLRefactorConfig) -> None:
        self.sql_str = content.current_str
    ...
    def execute(self) -> SQLRuleRefactorResult:
        ...
        return SQLRuleRefactorResult(
            ...
            refactored_content=self.sql_str,
            original_content=self.sql_str,
            deprecation_refactors=self._deprecation_refactors,
