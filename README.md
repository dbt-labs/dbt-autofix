...
- `dbt-autofix deprecations`: refactor YAML and SQL files to fix some deprecations
  ...
  - add `--incude-private-packages` to autofix just the _private_ packages installed. Just note that those fixes will be reverted at the next `dbt deps` and the long term fix will be to update the packages to versions compatible with Fusion.
