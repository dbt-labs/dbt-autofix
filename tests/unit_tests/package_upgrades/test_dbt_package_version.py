...
@pytest.mark.parametrize(
    ...
    [
        (
            "0.8.7",
            VersionSpecifier(major="0", minor="8", patch="7", matcher=Matchers.EXACT)
        ),
        (
            "1.7.10-beta",
            VersionSpecifier(major="1", minor="7", patch="10", prerelease="beta", matcher=Matchers.EXACT)
        )
    ]
)
...
version_1_1_0_ge: VersionSpecifier = VersionSpecifier(major="1", minor="1", patch="0", matcher=Matchers.GREATER_THAN_OR_EQUAL)
version_1_1_0_lt: VersionSpecifier = VersionSpecifier(major="1", minor="1", patch="0", matcher=Matchers.LESS_THAN)
version_1_1_0_le: VersionSpecifier = VersionSpecifier(major="1", minor="1", patch="0", matcher=Matchers.LESS_THAN_OR_EQUAL)
version_2_0_0_lt: VersionSpecifier = VersionSpecifier(major="2", minor="0", patch="0", matcher=Matchers.LESS_THAN)
version_2_0_0_le: VersionSpecifier = VersionSpecifier(major="2", minor="0", patch="0", matcher=Matchers.LESS_THAN_OR_EQUAL)
version_3_0_0_lt: VersionSpecifier = VersionSpecifier(major="3", minor="0", patch="0", matcher=Matchers.LESS_THAN)
version_3_0_0_le: VersionSpecifier = VersionSpecifier(major="3", minor="0", patch="0", matcher=Matchers.LESS_THAN_OR_EQUAL)
...
@pytest.mark.parametrize(
    ...
    [
        (
            {'name': 'dbt_set_similarity', 'version': '0.1.0', 'require-dbt-version': ['>=1.1.0', '<2.0.0'], 'config-version': 2, 'model-paths': ['models'], 'target-path': 'target', 'clean-targets': ['target', 'dbt_modules'], 'macro-paths': ['macros'], 'log-path': 'logs'},
            VersionRange(version_1_1_0_ge, version_2_0_0_lt)
        ),
        (
            {'name': 'dbt_set_similarity', 'version': '0.1.0', 'config-version': 2, 'model-paths': ['models'], 'target-path': 'target', 'clean-targets': ['target', 'dbt_modules'], 'macro-paths': ['macros'], 'log-path': 'logs'},
            None
        ),
        (
            {'name': 'dbt_set_similarity', 'version': '0.1.0', 'require-dbt-version': ['>=1.1.0'], 'config-version': 2, 'model-paths': ['models'], 'target-path': 'target', 'clean-targets': ['target', 'dbt_modules'], 'macro-paths': ['macros'], 'log-path': 'logs'},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {'name': 'dbt_set_similarity', 'version': '0.1.0', 'require-dbt-version': ['>=1.1.0', '<3.0.0'], 'config-version': 2, 'model-paths': ['models'], 'target-path': 'target', 'clean-targets': ['target', 'dbt_modules'], 'macro-paths': ['macros'], 'log-path': 'logs'},
            True
        ),
        (
            {'name': 'dbt_set_similarity', 'version': '0.1.0', 'require-dbt-version': ['<3.0.0'], 'config-version': 2, 'model-paths': ['models'], 'target-path': 'target', 'clean-targets': ['target', 'dbt_modules'], 'macro-paths': ['macros'], 'log-path': 'logs'},
            True
        ),
