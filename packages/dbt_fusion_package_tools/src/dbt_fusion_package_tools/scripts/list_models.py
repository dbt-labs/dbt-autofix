    ...
    for i, package in enumerate(output):
        ...
        for version in output[package]:
            ...
            if package_version_download_url is None:
                ...
                continue
            ls_output: list[str] = download_tarball_and_run_ls(
            ...
            if ls_output is None or len(ls_output) == 0:
                console.log(f"Could not run ls for {package} version {package_version_string}\n")
                continue
            else:
                results[package][package_version_string] = ls_output
                console.log()

    return results


def write_ls_output_to_csv(
...
def main(
    ...
    console.log(f"Writing to output path: {output_dir}/ls_output.json")
    ...
    ls_results: dict[str, dict[str, list[str]]] = run_ls_from_tarballs(
    ...
    # console.log(f"Successfully wrote output to {output_path}/ls_output.json")
    ...
    with open(Path(output_path) / "model_summary.csv", mode="w") as file:
