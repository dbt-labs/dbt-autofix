...
from dbt_package_tools.dbt_package_version import DbtPackageVersion
from dbt_package_tools.upgrade_status import PackageVersionFusionCompatibilityState
from dbt_package_tools.version_utils import VersionSpecifier
...
def main():
    input: Path = Path.cwd() / "src" / "dbt_package_tools" / "scripts" / "output"
    ...
    with open(
        Path.cwd() / "src" / "dbt_package_tools" / "fusion_version_compatibility_output.py",
        "w",
    ) as output_py_file:
